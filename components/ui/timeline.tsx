...
export const TimelineItem : FC<TimelineItemProps> = ({date , title , subTitle, link , tag, isCourse}) =>{
    return <div className="flex flex-wrap gap-12 min-h relative">
        {/* Date timeline */}
        ...
        <div className="flex gap-x-2" style={{transform:`${isCourse ? "translateX(-45px)" : ""}`}}>
            <div className="flex flex-col gap-0.5">
                ...
                <div className="flex items-center gap-2 w-min">
                    {
                        link ? <Link href={link} target="_blank"><Body link={link} subTitle={subTitle} tag={tag} /></Link> : <Body subTitle={subTitle} tag={tag} />
                    }
                </div>
            </div>
        </div>
